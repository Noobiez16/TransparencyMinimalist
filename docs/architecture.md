...
- `src/engine/selection.ts` holds the pixel selection as an ordered list of operations (shapes with a boolean mode, plus Select All and Invert); the document-sized mask canvas, its alpha array, and its bounds are derived caches rebuilt whenever the list or the document size changes. Storing operations rather than pixels is what makes selection changes undoable: a full-resolution mask snapshot would cost megabytes per history entry, while an operation list costs bytes. Boolean combination is expressed directly as canvas composite operations — `source-over` to add, `destination-out` to subtract, `destination-in` to intersect — so arbitrary lasso shapes combine correctly without polygon algebra. `src/engine/selection-contour.ts` walks the mask's pixel lattice to produce closed outlines for the marching-ants overlay, and `src/engine/selection-edit.ts` clears, fills, and crops through the same dirty-rect command pattern the stroke engine uses.
- Selection-constrained editing bridges two coordinate spaces: the mask lives in document space while layer bitmaps live in their own pixel space. `documentToBitmapMatrix` supplies the inverse affine as a `setTransform` tuple, so the mask can be drawn into a bitmap-space scratch canvas and applied with `destination-in` — clipping brush strokes, Clear, and Fill to the selection without changing any downstream undo behavior.

- `src/engine/shape-geometry.ts` is the vector-shape core. `shapeCommands` turns a shape's parameters (rectangle with corner radius, ellipse, line, regular polygon) into a list of drawing commands in the layer's local space, `shapeNaturalSize` derives its bounding box, and `constrainDragRect` / `constrainLine` apply the Shift and Alt drag constraints. These are plain data structures rather than `Path2D` objects specifically so they can be unit-tested in Node. Adding a third layer kind touched surprisingly little: because `layerNaturalSize` reports a shape's size from its parameters, Free Transform, smart-snapping guides, crop, and hit-testing all work on shape layers without changes. `layerNaturalSize` also floors a shape's size at its stroke width, so a perfectly horizontal line still has a grabbable box.
- `src/engine/shape-raster.ts` converts a shape layer to pixels for `Layer > Rasterize Shape`, replaying the command list into a new bitmap and swapping the layer's kind while keeping its id, name, transform, and effects, so history and panel selection keep pointing at the same layer.

- `src/engine/path-model.ts` defines pen paths: a path is a list of subpaths whose anchors carry their handles as offsets relative to the anchor, so a **corner anchor is simply one whose handles have zero length** — there is no separate corner/smooth flag that can fall out of step with the geometry, and moving an anchor carries its handles for free. `path-geometry.ts` is the pure core: it converts subpaths into drawing commands, hit-tests anchors, handles, and segments, and performs the editing operations, including a De Casteljau split so inserting an anchor never changes the curve's shape. `path-store.ts` keeps paths on the document (`doc.paths`) and routes every mutation through a single history command, and `path-ops.ts` bridges paths to the other engines: convert to a shape layer, load as a pixel selection, fill or stroke onto an image layer, and trace a selection back into a work path.
- **Paths are non-printing.** They are drawn by `src/canvas-overlay.ts` and never by the compositor, so they guide work on screen without ever reaching an exported PNG — matching Photoshop, where a path becomes pixels only when you convert, fill, or stroke it.
- `src/engine/path-render.ts` holds the only `PathCommand` switch in the codebase. Shapes, rasterization, layer thumbnails, and the overlay all replay commands through it, so adding a command variant cannot silently draw nothing in a site that forgot to handle it — a gap TypeScript would not catch, since a non-exhaustive switch is legal.

- `src/engine/text-model.ts` stores a text layer's styling as a flat string plus **style spans** — sorted, contiguous ranges that cover the whole string, repaired by `normalizeSpans` rather than trusted from callers. The flat-string shape is deliberate: a caret is then a single integer index and a selection a plain range, which is what the forthcoming on-canvas editor needs. `applyStyleToRange` splits and re-merges spans, and every style edit in the app — options bar, Properties, and the foreground colour chip — goes through it, so the whole-layer case already exercises the path a selection will later narrow.
- `src/engine/text-layout.ts` lays out point type as a pure function over an injected character-measurement callback, which is what makes line breaking, alignment, leading, and tracking testable in Node without a canvas. A line's height is the maximum leading among its runs, and its baseline sits at `0.8 × maxFontSize` below the line top — the standard ascent approximation available without font metrics, and the reason mixed sizes on one line share a baseline. `layerNaturalSize` and the compositor both read this layout, so transform, snapping, and hit-testing needed no changes.
- `src/engine/text-raster.ts` rasterizes a text layer to pixels and converts one to a vector shape. The conversion renders at 2× and traces the alpha with Phase C's contour walker: Canvas exposes no glyph outlines and the project carries no font-parsing dependency, so these are **traced outlines**, clean at display sizes and stair-stepped on small text.
