...
export const MENUS: Array<{ title: string; items: Array<string | '—'> }> = [
  ...
  { title: 'Layer', items: ['layer.newImage', 'layer.newText', '—', 'layer.duplicate', 'layer.rasterizeShape', 'path.convertToShape', 'path.fill', 'path.stroke', 'layer.delete', 'layer.group'] },
  { title: 'Type', items: ['type.rasterize', 'type.convertShape'] },
  { title: 'Select', items: ['select.all', 'select.deselect', 'select.reselect', 'select.inverse', 'path.loadAsSelection', 'path.makeWorkPath', 'select.subject'] },
